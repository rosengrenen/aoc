...
impl Solver for Day7Solver {
  fn solve(&self, lines: Vec<String>, part_two: bool) -> String {
    ...
    if part_two {
      ...
      permutations.iter().fold(0, |previous, permutation| {
...
fn run_sequence(program: &mut Vec<i32>, sequence: &Vec<i32>, part_two: bool) -> i32 {
  let mut result = 0;
  if part_two {
    let mut index = 0;
...
impl IntcodeComputer {
  ...
  fn get_n_args(&self, n: usize) -> Vec<(i32, i32)> {
    let mut shift = 100;
