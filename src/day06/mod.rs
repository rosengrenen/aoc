...
impl Solver for Day6Solver {
  fn solve(&self, lines: Vec<String>, part_two: bool) -> String {
    ...
    if part_two {
      ...
      calculate_orbit_counts(&tree).to_string()
    }
  }
}
...
fn find_path(node: &OrbitNode, target: String) -> Vec<(String, i32)> {
  let mut path: Vec<(String, i32)> = Vec::new();
  find_path_builder(node, target, &mut path);
  path
}

fn find_path_builder(node: &OrbitNode, target: String, acc: &mut Vec<(String, i32)>) -> bool {
  if node.name == target {
    return true;
  }
...
fn calculate_orbital_transfers(node: &OrbitNode, from: String, to: String) -> i32 {
