...
impl Solver for Day10Solver {
  fn solve(&self, lines: Vec<String>, part_two: bool) -> String {
    let asteroid_map = create_asteroid_map(&lines);
    let (best_asteroid_position, asteroids_in_sight) = find_best_asteroid(&asteroid_map);
    if part_two {
      let vaporized_asteroid_200 =
        vaporize_asteroids(asteroid_map.clone(), &best_asteroid_position)[199];
      (vaporized_asteroid_200.x * 100 + vaporized_asteroid_200.y).to_string()
    } else {
      asteroids_in_sight.to_string()
    }
  }
}
