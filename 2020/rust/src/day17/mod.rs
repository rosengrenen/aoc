            ...
            'delta: for &dx in RANGE.iter() {
              for &dy in RANGE.iter() {
                for &dz in RANGE.iter() {
                  ...
                  if ax < lo_x - ITERATIONS as i64 || ax > hi_x + ITERATIONS as i64 {
                    continue;
                  }

                  if ay < lo_y - ITERATIONS as i64 || ay > hi_y + ITERATIONS as i64 {
                    continue;
                  }

                  if az < lo_z - ITERATIONS as i64 || az > hi_z + ITERATIONS as i64 {
                    continue;
                  }
        ...
        for y in lo_y - i - 1..=hi_y + i + 1 {
          for z in lo_z - i - 1..=hi_z + i + 1 {
            for w in lo_w - i - 1..=hi_w + i + 1 {
              let active = get_xyzw(&space, (x_size, y_size, z_size, w_size), (x, y, z, w));
              let mut count = 0;
              'delta: for &dx in RANGE.iter() {
                for &dy in RANGE.iter() {
                  for &dz in RANGE.iter() {
                    for &dw in RANGE.iter() {
                      ...
                      if aw < lo_w - ITERATIONS as i64 || aw > hi_w + ITERATIONS as i64 {
                        continue;
                      }

                      if get_xyzw(&space, (x_size, y_size, z_size, w_size), (ax, ay, az, aw)) {
                        count += 1;
                      }
                      if count > 3 {
                        break 'delta;
